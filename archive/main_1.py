...
#         # Display metadata table with a 'View Details' button for each file
#         for index, row in df_metadata.iterrows():
#             st.write(f"**Description**: {row['description']}")
#             st.write(f"**Author**: {row['author']}")
            
#             # Create a button to view details for each file
#             if st.button(f"View Details for {row['_id']}", key=f"view_{row['_id']}"):
#                 # Store the selected file ID in session state to display details later
#                 st.session_state.selected_file_id = row['_id']
    
#     else:
#         st.write("No data available.")
    
#     # Display file details if a file has been selected
#     if 'selected_file_id' in st.session_state:
#         st.write("### File Details:")
#         afficher_details_fichier(st.session_state.selected_file_id)
...
#         # Select relevant columns for display
#         df_metadata = df_metadata[['description', 'author', 'start_date', '_id']]
...
#             if st.button(f"View Details - {row['_id']}", key=index):
#                 st.session_state.file_id = str(row['_id'])
#                 st.experimental_rerun()
    
#     # Handle file detail display if file_id is in session state
...
#         # Add "View Details" button to DataFrame
#         df_metadata['View Details'] = df_metadata.apply(
#             lambda row: f'<a href="?file_id={str(row["_id"])}" target="_self">View Details</a>', axis=1
#         )
...
#             'text-align': 'left'
#         }).format({'View Details': lambda x: x}, escape="html")
        
#         # Display the styled DataFrame
#         st.write(df_styled.to_html(escape=False, index=False), unsafe_allow_html=True)
        
#     # Handle file detail display if a file_id is provided
#     query_params = st.query_params
#     if "file_id" in query_params:
#         file_id = query_params["file_id"][0]
#         if ObjectId.is_valid(file_id):  # Validate ObjectId
