...
# Fonction pour générer un ID de fichier basé sur l'utilisateur et l'année
...
def update_data_in_mongo(file_id, updates):
    try:
        # Impression pour déboguer
        print(f"Received updates: {updates}")
        print(f"Type of updates: {type(updates)}")

        # Vérifiez que 'updates' est un dictionnaire
        if not isinstance(updates, dict):
            raise ValueError("Updates must be a dictionary.")
        
        # Assurez-vous que le champ '_id' n'est pas inclus dans les mises à jour
        updates.pop('_id', None)
        
        # Vérifiez si le dictionnaire de mises à jour est vide
        if not updates:
            st.warning("No fields to update.")
            return

        # Mise à jour du document avec le dictionnaire
...
# Fonction pour mettre à jour les données à partir d'un fichier avec des colonnes spécifiques
def upload_by_update(file):
    try:
        ...
        # Générer un nouvel ID de fichier pour le traitement
        file_id = generate_file_id('admin')  # Vous pouvez adapter le nom de l'utilisateur si nécessaire
        df['file_id'] = file_id

        # Convertir les DataFrame en dictionnaires pour MongoDB
        records = df.to_dict('records')

        # Supprimer les documents existants basés sur les critères des colonnes
...
if not st.session_state.logged_in:
    ...
    if page == "Home":
        ...
        st.write("""
            ...
            - **Upload by Update**: Upload a file to update existing records.
