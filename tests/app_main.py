...
if not st.session_state.logged_in:
    ...
    # # # Menu de navigation
    # menu = st.sidebar.selectbox("Connexion", ["Welcome", "Login"])
    # if menu == "Welcome":
    #     st.button("Discover more")
    # else:
    #     log_user()

    # Create the sidebar menu using streamlit_option_menu
    with st.sidebar:
        menu_option = option_menu(
            menu_title="Connexion",  # Set the title of the menu
            options=["Welcome", "Login", "view Data"],  # List the options
            icons=["house", "person", "file-text"],  # Optional: set icons for the menu items
            default_index=0,  # Optional: set the default selected option
            orientation="vertical"  # Optional: vertical or horizontal layout
        )
    ...
    if menu_option == "Welcome":
        ...
        log_user()  # Ensure that this function is defined elsewhere in your code
    ...
    with st.sidebar:
        page = option_menu("Big Data", 
            ["🏠 Home", "📁 Upload Data", "📂View Data", "📝Update Data", "📂View Metadata", "🗑️ Delete a File",  "🔍 Requete sur données", "⬇️ Télécharger Données public","🚪Logout"],
