...
# # Fonction pour obtenir la liste des fichiers disponibles
# def get_file_metadata():
#     return list(metadata_collection.find({}, {'_id': 1, 'description': 1}))
...
#         if metadata:
#             file_options = {m['description']: m['_id'] for m in metadata}
#             selected_file = st.selectbox("Sélectionnez un fichier", options=list(file_options.keys()))

#             if selected_file:
#                 file_id = file_options[selected_file]
#                 data = list(collection.find({'_id': file_id}))
        ...
        try:
            ...
            if metadata:
                ...
                if selected_file_id:
                    ...
                    data = list(collection.find({'_id': {'$in': [doc['_id'] for doc in collection.find({'_id': selected_file_id})]}}))
                    if data:
                        df = pd.DataFrame(data)
                        st.dataframe(df)
                        # Télécharger les données
