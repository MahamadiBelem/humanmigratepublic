...
# Fonction principale pour l'affichage de l'utilisateur
def main():
    st.subheader(f"Bonjour {keycloak.user_info.get('preferred_username', 'User')}!")
    if st.button("Déconnexion"):
        disconnect()

# Initialiser la connexion à Keycloak
...
# Interface de l'application en fonction de l'état d'authentification
if st.session_state.authenticated:
 ...
 # Configuration du menu dans la barre latérale
    menu_options = ["Welcome dashboard", "Charger un fichier","Mettre à jour un fichier","Supprimer un fichier", "🔍 API", "🔍 Prediction"]
    if st.session_state.authenticated:
    ...
    with st.sidebar:
        choix = option_menu(
            "Menu", 
            menu_options,
            icons=["house", "cloud-upload", "pencil", "trash", "info-circle"],
            menu_icon="cast",
            default_index=0,
            orientation="vertical"
        )

    # Fonctionnalités basées sur la sélection du menu
    if choix == "Welcome dashboard":
        home_admin()
    elif choix == "Charger un fichier":
        charger_fichier()
...
menu_options = ["Welcome dashboard", "🔍 API","🔍Prediction"]
...
if keycloak.authenticated:
    with st.sidebar:
        choix = option_menu(
            "Menu", 
            menu_options,
            icons=["house", "cloud-upload", "pencil", "trash",  "info-circle"] if keycloak.authenticated else ["house", "table","table","info-circle"],
            menu_icon="cast",
            default_index=0,
            orientation='vertical'
        )
